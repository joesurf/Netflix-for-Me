...
class VideoDetect:
    """
    Moderates video content with labels using Amazon Rekognition
    """
    ...
    def GetUnsafeContentResults(self):
        ...
        while finished == False:
            ...
            metadata['Codec'] = response['VideoMetadata']['Codec']
            metadata['Duration'] = str(response['VideoMetadata']['DurationMillis'])
            metadata['Format'] = response['VideoMetadata']['Format']
            metadata['Frame rate'] = str(response['VideoMetadata']['FrameRate'])
            ...
            for contentModerationDetection in response['ModerationLabels']:
                ...
                if (str(contentModerationDetection['ModerationLabel']['ParentName']) not in unsafeContentLabels
                and contentModerationDetection['ModerationLabel']['Confidence'] >= 80):
                    unsafeContentLabels.append(str(contentModerationDetection['ModerationLabel']['ParentName']))
        ...
        return {
            "unsafeContentLabels": unsafeContentLabels,
            "metadata": metadata
        }
        # for label in unsafeContentLabels:
        #     print(label)
...
def lambda_handler(event, context):
    ...
    roleArn = 'arn:aws:iam::822767915126:role/RekognitionVideoModeration'
    bucket = 'nflx-upload-storage'
    video = 'dirtyTestVideo.mp4'

    session = boto3.Session(profile_name='nflx', region_name='us-east-1')
